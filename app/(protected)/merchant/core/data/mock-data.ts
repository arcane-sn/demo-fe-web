...
export const getMerchantsForReview = (): MerchantData[] => {
  return mockMerchants.filter(merchant => 
    ['draft', 'pending-review', 'approved', 'rejected'].includes(merchant.reviewStatus)
  );
};

export const getMerchantsForList = (): MerchantData[] => {
  return mockMerchants.filter(merchant => merchant.reviewStatus === 'approved');
};
...
export const getMerchantsByLevel = (level: number): MerchantData[] => {
  return mockMerchants.filter(merchant => merchant.merchantLevel.level === level);
};

export const getSubMerchants = (parentId: string): MerchantData[] => {
  return mockMerchants.filter(merchant => merchant.merchantLevel.parentMerchantId === parentId);
};
...
export const getMockOthersData = () => ({
  ...
  additionalNotes: {
    notes: 'Lorem ipsum dolor sit amet consectetur. Cursus gravida sed dignissim cursus turpis ut. Lectus facilisis volutpat venenatis odio fusce adipiscing justo pharetra. Commodo vitae commodo sit amet commodo pellentesque molestie egestas volutpat. Id non convallis pharetra in orci elit pharetra pretium.'
  }
});
