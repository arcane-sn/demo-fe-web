...
export class MerchantService {
  ...
  static async getMerchantReviewsByTab(
    tabId: string
  ): Promise<MerchantReviewData[]> {
    await new Promise((resolve) => setTimeout(resolve, 100));
    return getMerchantReviewsByTabForReview(tabId);
  }

  static async createMerchant(
    merchantData: Omit<MerchantData, "id">
  ...
  static validateMerchantData(data: Partial<MerchantData>): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    if (
      data.phoneNumber &&
      !/^(\+62|62|0)[0-9]{9,13}$/.test(data.phoneNumber)
    ) {
    ...
    try {
      ...
      if (isServerSide()) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to fetch merchant metrics: ${response.statusText}`
            );
          }
    ...
    try {
      ...
      if (isServerSide()) {
        ...
        try {
          ...
          if (!response.ok) {
            throw new Error(
              `Failed to fetch merchant activity: ${response.statusText}`
            );
          }
  ...
  static async bulkRejectMerchants(
    ids: string[],
    reason?: string
  ): Promise<MerchantData[]> {
    const results: MerchantData[] = [];
