...
export function useSectionNavigation() {
  ...
  const getCurrentSectionIndex = useCallback(() => {
    ...
    // After 'edit', the next part is the merchant ID, and the part after that is the section
    const currentSection = editIndex >= 0 && editIndex + 2 < pathParts.length 
      ? pathParts[editIndex + 2] 
      : '';
  ...
  const goToPrevious = useCallback(() => {
    ...
    if (currentIndex > 0 && merchantId) {
      ...
      const targetPath = `/merchant/list/edit/${merchantId}${previousSection.path}`;
  ...
  const goToNext = useCallback(() => {
    ...
    if (currentIndex < EDIT_SECTIONS.length - 1 && merchantId) {
      ...
      const targetPath = `/merchant/list/edit/${merchantId}${nextSection.path}`;
      router.push(targetPath);
    }
  }, [getCurrentSectionIndex, getMerchantId, router]);
