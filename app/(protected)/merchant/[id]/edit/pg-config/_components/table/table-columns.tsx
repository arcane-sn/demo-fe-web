...
export function getTableColumns(options: TableColumnsOptions = {}): ColumnDef<ChannelData>[] {
  ...
  const actions: TableActionConfig<ChannelData> | undefined = showActions ? {
    showActions: true,
    onEdit: onEditChannel ? (channel) => onEditChannel(channel.id) : undefined,
    onDelete: onDeleteChannel && showDeleteAction ? (channel) => onDeleteChannel(channel.id) : undefined,
    showDeleteAction,
  } : undefined;

  return createTableColumns(columns, actions);
}