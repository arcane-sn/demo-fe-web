...
export function CheckboxList({
  ...
  const renderOption = (option: CheckboxOption, currentLevel = 0) => {
    ...
    if (currentLevel === 0 && hasChildren) {
      ...
      // Simple main category without children (like Payment Status, Activity, Provider Name)
      // No toggle button needed since there are no nested children
      return (
        <div key={option.id} className="flex flex-col gap-3 w-full">
          <div
            className="flex items-center gap-1.5 cursor-pointer"
